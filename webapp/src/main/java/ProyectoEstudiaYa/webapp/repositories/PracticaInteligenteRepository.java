...
public interface PracticaInteligenteRepository extends JpaRepository<EjercicioEntity, Long> {
     ...
     */
    @Query("SELECT e FROM EjercicioEntity e JOIN FETCH e.tema t JOIN FETCH t.curso WHERE e.tema.id = :temaId ORDER BY e.dificultad ASC")
    List<EjercicioEntity> findByTemaId(@Param("temaId") Long temaId);

    /**
     * Busca ejercicios por nivel de dificultad
     */
    @Query("SELECT e FROM EjercicioEntity e JOIN FETCH e.tema t JOIN FETCH t.curso WHERE e.dificultad = :dificultad ORDER BY e.tema.id ASC")
     ...
     */
    @Query("SELECT e FROM EjercicioEntity e JOIN FETCH e.tema t JOIN FETCH t.curso WHERE e.generadoPorIA = :generadoPorIA ORDER BY e.dificultad ASC")
     ...
     */
    @Query("SELECT e FROM EjercicioEntity e JOIN FETCH e.tema t JOIN FETCH t.curso WHERE e.tema.id = :temaId AND e.dificultad = :dificultad ORDER BY e.id ASC")
     ...
     */
    @Query("SELECT e FROM EjercicioEntity e JOIN FETCH e.tema t JOIN FETCH t.curso WHERE t.curso.id = :cursoId ORDER BY t.id ASC, e.dificultad ASC")
     ...
     */
    @Query("SELECT e FROM EjercicioEntity e JOIN FETCH e.tema t JOIN FETCH t.curso WHERE e.tema.id = :temaId AND SIZE(e.intentos) = 0")
    List<EjercicioEntity> findNoResolvedByTemaId(@Param("temaId") Long temaId);

    /**
     * Busca ejercicios de múltiples temas con tema y curso cargados eagerly
     */
    @Query("SELECT e FROM EjercicioEntity e JOIN FETCH e.tema t JOIN FETCH t.curso WHERE t.id IN :temaIds")
    List<EjercicioEntity> findByTemaIdsInWithTemaAndCurso(@Param("temaIds") List<Long> temaIds);
}
